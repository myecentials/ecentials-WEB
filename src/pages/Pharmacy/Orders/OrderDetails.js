...
const OrderDetails = () => {
	...
	const columns = [
		...
		},
		{
			...
			cell: (row) => (
				<span>
					{(
						row.prize * row.quantity -
						row.discount * 0.01 * (row.prize * row.quantity)
					).toFixed(2)}
				</span>
			),
		},
	...
	const handleCancelOrder = async () => {
		handleModalClose();
		const load = toast.loading("Cancelling order ...")
		try {
	...
	for (let total of products) {
		sum +=
			total.prize * total.quantity -
			total.discount * 0.01 * (total.prize * total.quantity);
	}

	const handleProcessOrder = async () => {
		setIsOpenComfirm(false)
		const load = toast.loading("Processing order...")
